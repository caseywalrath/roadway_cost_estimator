...
- Schema v8 stores each Project independently in IndexedDB. Each Project stores a non-negative `contingencyPercent`; each line identifies its origin as `explorer`, `catalog`, or `custom` and stores an optional Project-specific `group`.
- Explorer-backed lines retain `state`, `agencyId`, `agencyItemId`, and evidence context. Catalog-linked lines retain official agency identity without claiming an evidence snapshot. Custom lines accept free-form group, item code, description, unit, notes, and nullable Unit Cost/Quantity without catalog identity or evidence context.
...
- The Project Items header shows Construction bid-item cost, Other Costs, a Project-level editable contingency percentage and calculated contingency amount, plus Total Project Cost in one horizontal summary strip. Construction is the sum of Explorer-backed and catalog-linked lines; Other Costs is the sum of custom lines; contingencies apply to their combined base; blank or legacy percentages are zero.
