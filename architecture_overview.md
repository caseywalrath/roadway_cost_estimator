...
- Brand assets live in `public/brand/` (copied to the site root at build time): `FHU-logo.png` (the firm's official horizontal lockup) and `favicon.svg` (the road mark on a rounded tile). The logo is referenced from the header in `src/ui/renderApp.ts` and the status panels in `src/main.ts`. Static assets must live under `public/` to be served by the Vite build; a root-level file is not bundled.
