...
data/staging/ne/item_identity_resolutions.csv
```

Nebraska must not be enabled while `item_identity_conflicts.csv` contains unresolved material conflicts.

The resolution file is generated by the audit and is the review handoff. It records conservative automatic classifications such as `normalized_equivalent` and `stable_description_multi_unit`; automatic classifications do not authorize semantic identity merges. Rows that remain materially different retain `resolution_status=needs_review` until an engineer records the source evidence and final action.
